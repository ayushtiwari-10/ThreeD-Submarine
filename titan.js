let scene, camera, renderer, submarine, ship;
        ...
        function init() {
            ...
            const oceanMaterial = new THREE.MeshPhongMaterial({ color: 0x006994, transparent: true, opacity: 0.8 });
            ...
            const floorMaterial = new THREE.MeshPhongMaterial({ color: 0x00000, side: THREE.DoubleSide });
            ...
            document.getElementById('lets-go-button').addEventListener('click', startExploration);
            document.getElementById('read-info-button').addEventListener('click', showTitanicInfo);
        ...
        function loadModels() {
            ...
            loadModel('models/submarine/scene.gltf', (gltf) => {
            ...
            loadModel('models/cruise_ship/scene.gltf', (gltf) => {
                ship = gltf.scene;
            ...
            loadModel('models/titanic__wreck/scene.gltf', (gltf) => {
        ...
        function addSpotlightsToSubmarine() {
            ...
            const createSpotlight = (x, y, z) => {
                const spotlight = new THREE.SpotLight(0xffffff, spotlightIntensity, spotlightDistance, spotlightAngle, spotlightPenumbra);
        ...
        function onMouseMove(event) {
            ...
            submarine.rotation.x = Math.max(-Math.PI / 2, Math.min(Math.PI / 2, submarine.rotation.x));
    ...
    submarine.position.set(0, -10, 0);
    camera.position.set(submarine.position.x, submarine.position.y + 10, submarine.position.z + 30);
...
function updateEnvironment() {
    ...
    // Any other reset logic
        ...
        function animate() {
            ...
            if (modelsLoaded) {
                if (exploring && !isImploding) {
                    ...
                    if (moveLeft) submarine.position.add(new THREE.Vector3(-direction.z, 0, direction.x).normalize().multiplyScalar(speed));
                    if (moveRight) submarine.position.add(new THREE.Vector3(direction.z, 0, -direction.x).normalize().multiplyScalar(speed));if (moveUp) submarine.position.y += speed;
                    if (moveDown) submarine.position.y -= speed;

                    submarine.position.y = Math.max(Math.min(submarine.position.y, MIN_DEPTH), MAX_DEPTH);

                    camera.position.copy(submarine.position).add(new THREE.Vector3(0, 10, 30).applyQuaternion(submarine.quaternion));
                    camera.lookAt(submarine.position);
                    ...
                    marineLife.forEach(animal => {
                        ...
                        animal.model.position.x = (animal.model.position.x + 4000) % 8000 - 4000;
                        animal.model.position.z = (animal.model.position.z + 4000) % 8000 - 4000;
        ...
        function returnToMap() {
            ...
            window.location.href = `threeD.html?returnedFrom=${encodeURIComponent(currentLocation.name)}`;
        }
